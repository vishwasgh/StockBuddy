...
export default function CryptoPredictor({ symbol }: CryptoPredictionProps) {
  ...
  useEffect(() => {
    const fetchPredictionData = async () => {
      try {
        const response = await fetch(`http://localhost:2000/stock-prediction?predictor=${activePredictor}&ticker=${symbol}&interval=1hr&api=binance&days_ahead=30`);
        if (!response.ok) {
  ...
  return (
    <Card className="shadow-lg p-4">
      <CardHeader>
        <CardTitle>Price Prediction for {symbol}</CardTitle>
      </CardHeader>
      <CardContent>
